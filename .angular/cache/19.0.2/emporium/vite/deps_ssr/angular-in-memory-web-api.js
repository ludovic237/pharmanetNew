...
var BackendService = class {
  ...
  get dbReady() {
    if (!this.dbReadySubject) {
      this.dbReadySubject = new import_rxjs.BehaviorSubject(false);
      this.resetDb();
    }
    return this.dbReadySubject.asObservable().pipe((0, import_operators.first)((r) => r));
  }
  /**
   ...
   */
  handleRequest(req) {
    return this.dbReady.pipe((0, import_operators.concatMap)(() => this.handleRequest_(req)));
  }
  handleRequest_(req) {
  ...
  commands(reqInfo) {
    ...
    switch (command) {
      case "resetdb":
        ...
        return this.resetDb(reqInfo).pipe((0, import_operators.concatMap)(() => this.createResponse$(
          () => resOptions,
          false
          /* no latency delay */
        )));
  ...
  createResponseOptions$(resOptionsFactory) {
    return new import_rxjs.Observable((responseObserver) => {
      let resOptions;
      try {
  ...
  resetDb(reqInfo) {
    ...
    const db$ = db instanceof import_rxjs.Observable ? db : typeof db.then === "function" ? (0, import_rxjs.from)(db) : (0, import_rxjs.of)(db);
    db$.pipe((0, import_operators.first)()).subscribe((d) => {
      this.db = d;
...
var HttpClientBackendService = class _HttpClientBackendService extends BackendService {
  ...
  createResponse$fromResponseOptions$(resOptions$) {
    return resOptions$.pipe((0, import_operators.map)((opts) => new HttpResponse(opts)));
  }
