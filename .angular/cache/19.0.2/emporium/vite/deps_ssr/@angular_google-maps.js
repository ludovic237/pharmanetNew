...
var MapEventManager = class {
  ...
  _pending = [];
  _listeners = [];
  _targetStream = new import_rxjs.BehaviorSubject(void 0);
...
var MapCircle = class _MapCircle {
  ...
  _ngZone = inject(NgZone);
  _eventManager = new MapEventManager(inject(NgZone));
  _options = new import_rxjs.BehaviorSubject({});
  _center = new import_rxjs.BehaviorSubject(void 0);
  _radius = new import_rxjs.BehaviorSubject(void 0);
...
var MapGroundOverlay = class _MapGroundOverlay {
  ...
  _ngZone = inject(NgZone);
  _eventManager = new MapEventManager(inject(NgZone));
  _opacity = new import_rxjs.BehaviorSubject(1);
  _url = new import_rxjs.BehaviorSubject("");
  _bounds = new import_rxjs.BehaviorSubject(void 0);
  _destroyed = new import_rxjs.Subject();
  _hasWatchers;
  /**
...
var MapInfoWindow = class _MapInfoWindow {
  ...
  _eventManager = new MapEventManager(inject(NgZone));
  _options = new import_rxjs.BehaviorSubject({});
  _position = new import_rxjs.BehaviorSubject(void 0);
...
var MapPolygon = class _MapPolygon {
  ...
  _eventManager = new MapEventManager(inject(NgZone));
  _options = new import_rxjs.BehaviorSubject({});
  _paths = new import_rxjs.BehaviorSubject(void 0);
  _destroyed = new import_rxjs.Subject();
  /**
...
var MapPolyline = class _MapPolyline {
  ...
  _eventManager = new MapEventManager(inject(NgZone));
  _options = new import_rxjs.BehaviorSubject({});
  _path = new import_rxjs.BehaviorSubject(void 0);
  _destroyed = new import_rxjs.Subject();
  /**
...
var MapTrafficLayer = class _MapTrafficLayer {
  _map = inject(GoogleMap);
  _ngZone = inject(NgZone);
  _autoRefresh = new import_rxjs.BehaviorSubject(true);
  _destroyed = new import_rxjs.Subject();
  /**
...
var MapAdvancedMarker = class _MapAdvancedMarker {
  ...
  /** Returns a promise that resolves when the marker has been initialized. */
  _resolveMarker() {
    return this.advancedMarker ? Promise.resolve(this.advancedMarker) : this.markerInitialized.pipe((0, import_operators.take)(1)).toPromise();
  }
  /** Creates a combined options object using the passed-in options and the individual inputs. */
  _combineOptions() {
...
var MapMarkerClusterer = class _MapMarkerClusterer {
  ...
  _markersSubscription = import_rxjs.Subscription.EMPTY;
  /** Whether the clusterer is allowed to be initialized. */
  _canInitialize = this._googleMap._isBrowser;
  /**
