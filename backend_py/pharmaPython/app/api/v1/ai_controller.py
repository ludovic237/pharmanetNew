...
def recommendations(for_produit_id: Optional[int] = None, limit_baskets: int = 5000, db: Session = Depends(get_db)):
  ...
  enriched = [Recommendation(produit_id=r["produit_id"], produit=id2nom.get(r["produit_id"]), score=r["score"]) for r in recos]
...
def optimize_dashboard(db: Session = Depends(get_db)):
  ...
  best = [Recommendation(produit_id=r["produit_id"], produit=id2nom.get(r["produit_id"]), score=r["score"]) for r in recos]

  # Low stock
  produits = ProduitRepository(db).find_top_n(limit=100)
  rows = [{"produit_id": p.id, "produit": p.nom, "stock": get_current_stock(db, p.id), "threshold": 5.0} for p in produits]
